...
public class JDBCTemp {
	...
	public List<Object> getList(String sqlid,Class clazz,Object...args){
		...
		String filename = "src/"+clazz.getName().replace(".", "/")+".xml";
		...
		try {
			...
			while(rs.next()){
				...
				for(String s:collectionmap.keySet()){
					String methodstr = "set"+Character.toUpperCase(s.charAt(0))+s.substring(1);
					Method method = Class.forName(collectionname).getDeclaredMethod(methodstr, Class.forName(collectionclass).getDeclaredField(s).getType());
		...
		for(int i=0;i<olist.size()-1;i++){
			...
			for(int j=i+1;j<olist.size();j++){
				...
				try {
					...
					Integer usersId0 = (Integer)method.invoke(olist.get(i));
					Integer usersId1 = (Integer)method.invoke(olist.get(j));
					String method000 = "get"+Character.toUpperCase(collectionname.charAt(0))+collectionname.substring(1);
					if(usersId0.equals(usersId1)){
